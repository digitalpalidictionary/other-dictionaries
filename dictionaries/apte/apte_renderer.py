"""Apte renderer — converts Cologne XML markup to HTML.
...
def inject_literature_tooltips(xml: str, author_tooltips: dict[str, str]) -> str:
    """Inject tooltip text into <ls> tags for literature references."""
...
def preprocess_xml(
    ...
    """Apply all pre-processing transformations to a single Apte entry."""
    xml = xml.replace("¦", " ")
...
_SCAN_URL_BASE = (
    ...
    "/scans/csl-apidev/servepdf.php?dict=AP90&page="
)
...
def render_xml_to_html(xml: str) -> str:
    ...
    Zero inline styles — all styling via CSS class names from apte.css.
    """
    ...
    if is_primary:
        ...
        if pc_value:
            ...
            scan_url = f"{_SCAN_URL_BASE}{page_num}"
            label += (
